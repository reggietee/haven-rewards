...
- Vitest: **30 tests pass** across engine, server synchronization, CSV, wheel math and offline operator authorization.
- Default prize pool: **34 unique finite units**, **8/8/9/9** hourly windows, **$5,319 CAD before HST**.
- Simulation: **250 spins**, **34 featured awards + 216 fallback passes**, no repeated unit, code or outcome; every configured finite quantity respected.
...
- Both engines: initialized and cached online, entered/spun offline without marketing consent, reloaded the recorded result without re-awarding, cleared personal UI state, unlocked controls offline using the PIN, entered with marketing consent, verified the actual 15-second reset, blocked normalized duplicate email, reloaded persisted inventory, reconnected and drained an idempotent mocked sync queue, and downloaded all five CSV types. Consents export includes both choices, identity linkage, email and exact proof.
...
The full-width gallery begins below the hero and includes 13 prize types, explicit Platinum/Gold/Silver/Bronze chapters, correct names/values/counts/sponsors and two Grand badges. No nested scrolling remains. Every card and the natural footer/QR is reachable; no horizontal overflow was detected. Counts include concealed future units. An actual award in an isolated fixture changes the total to 33 remaining and leaves the depleted card in place with `0 remaining`; the same Canvas survives entry, award and reset. Inactivity (20 seconds) and entrant reset return the document to the top.

Portrait (834 × 1112) stacks form, wheel and gallery with entry above the fold. At 1112 × 500, document scrolling keeps fields, both consent controls and the primary action reachable. The focused-field resize handling does not introduce a panel or sticky overlay. Platinum, Gold, Silver, Bronze, grayscale, sponsor backings and both orientations were visually inspected. Portrait wheel centering and the reduced-motion specificity of the Gold shimmer rule were corrected during QA. Rules/consent versions and stored schedules are unchanged. Canvas DPR remains capped at 1.5; no post-processing stack was added.
...
Vercel CLI reported `login_required` for the saved account. No remote deployment URL exists yet. The remaining deployment step is Vercel authentication, followed by project/environment configuration described in [README.md](../README.md) and [GOOGLE_SHEETS_SETUP.md](../GOOGLE_SHEETS_SETUP.md). No permanent Haven domain was touched. Public rules source is a draft requiring Haven’s final legal review.

Header/value follow-up: the header Haven mark renders white with no backing; the current 3-month membership value is $500 and the finite pool total is $5,319. Rules version is 2026-09-21.3. Untouched uninitialized $507 defaults upgrade with an audit record; initialized snapshots and operator-customized configurations are preserved. Build and 30 unit tests pass.
