...
- All 14 relevant browser cases pass across Chromium and WebKit: voice success/mute/failure/offline, manual Done, full offline kiosk and idempotent reconnect, grand animation, and early entry/7 p.m. closure. Initial test-only failures were corrected by muting before the short audio fixture ended and using an exact locator for duplicated closure copy; targeted reruns passed.
- Deployed real-provider recheck: HTTP 200 in **856 ms**, decoded **9.242-second** grand announcement played from the configured ElevenLabs endpoint before any reset. Exactly one generation marker and spin in the isolated test database; no real inventory consumed. Caption/replay absent; Done ends at **619 px** in the 834 px viewport and clears the result. See [live result](qa/voice-live-landscape.png).
...
Live Production endpoint: **real ElevenLabs generation and Web Audio playback pass** after the initial account HTTP 402 was resolved. A fictional grand-prize announcement arrived in **1,394 ms**, before the wheel stopped; its decoded audio lasted **7.848 seconds**. Exact caption, replay without a second request, mute, manual clearing, one saved spin and one generation marker were verified. The initial failure also exercised fallback. No real event inventory or personalized recording was created. See [voice setup](../VOICE_SETUP.md).

Final voice tests passed in both Chromium and WebKit. The request-phase assertion now polls its asynchronous observer, avoiding a test-helper race. Chromium checks were rerun without competing browser GPU work. Live visual QA found the cinematic logo overlapping the caption; its landscape reveal now sits over the wheel while the full-screen effects and timing remain unchanged.
