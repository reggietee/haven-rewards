...
5. The server request has a 7.5-second deadline; the client has an 8-second deadline. The normal wheel sequence is 6.8 seconds and reduced motion is 1.1 seconds. The result appears on schedule regardless.
6. Ready audio starts 500 ms after the result mounts. A valid response arriving later may begin once, up to 3 seconds after reveal; the result never waits. At that hard deadline the pending request is cancelled and late audio is discarded. This prevents short/reduced-motion spins from discarding otherwise healthy speech. Done, mute and backgrounding cancel the grace window immediately.
...
10. The booth panel preserves diagnostics such as authorization required, rate limited, service unavailable, invalid audio response, audio decode failed, request timed out, late and offline instead of masking them with a generic fallback status. Configuration readiness means variables exist, not that billing or voice access is valid.
